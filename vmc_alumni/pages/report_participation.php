...
<div class="content-box">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h3 class="fw-bold text-primary mb-0">Alumni Event Participation Report</h3>
        <button onclick="printEventTable()" class="btn btn-outline-primary">
            🖨️ Print
        </button>
    </div>
...
<script>
    function printEventTable() {
        ...
        // Clone the table
        let table = document.getElementById("eventTable").cloneNode(true);

        // Remove avatar images
        table.querySelectorAll("img").forEach(img => img.remove());

        let printWindow = window.open("", "", "width=1100,height=700");

        printWindow.document.write(`
        <html>
        <head>
            <title>Alumni Event Participation Report</title>
            <style>
                body {
        ...
        <body>
            <h2>Alumni Event Participation Report</h2>
            ${table.outerHTML}
        </body>
        </html>
    `);
